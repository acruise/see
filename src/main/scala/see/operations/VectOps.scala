...
private[see] class ReduceOp(opc: String)(val op: (Number, Number) => Number)
  ...
  private def check(v: Val) = {
    ...
    try {
      ...
      case _: Exception =>
        throw ParamError(s"Unsupported reduce operand: $r")
    }
  }
...
private[see] object Rep extends Unary("rep") {
  ...
  override def apply(s: Scope, args: Val): Val = args match {
    ...
    case _ =>
      throw new ParamError(s"$opCode requires (length, value).")
  }
  ...
  override def apply(s: Scope, args: Val): Val = args match {
    ...
    case Vector(Seq(src, ln, vn)) =>
      ...
      // apply here, so vector won't get stuffed with closures
        for (c <- buffer.size until len) {
          buffer += vn.coerce
        }
      Vector(buffer)

    case _ =>
      throw new ParamError(s"$opCode requires (source, length, value).")
  }
}
...
private[see] object Zip extends Unary("zip") {
    ...
    case Vector(vs) =>
      if (vs.size >= 2) {
        if (!vs.forall(_.isInstanceOf[Vector]))
          throw new ParamError(s"$opCode requires (vector1, vector2).")
        else Vector(merge(vs.asInstanceOf[List[Vector]]))
      }
      else if (vs.size == 1) // assume argument is list of vects
        apply(s, vs.head)
      else
        throw new ParamError(s"$opCode requires (vector1, vector2).")
    case _ =>
  ...
  private def merge(vects: Seq[Vector]): Seq[Val] = {
    val sz = (vects.head.size /: vects.tail)(_ min _.size)
...
private[see] object Slice extends Binary("@@") {
  ...
  override def apply(lhs: Val, rhs: Val): Val =
    (lhs, rhs) match {
      case (Str(x), Vector(l)) =>
        Str(mkslice(x, l).toString())

      case (Vector(x), Vector(l)) =>
        Vector(mkslice(x, l))
