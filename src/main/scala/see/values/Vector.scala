...
private[see] case class Vector(init: Seq[Val])
  ...
  override def at(index: Val) = index match {
    case n: Number =>
      ...
      if (i >= values.size || i < 0 && -i > values.size)
        throw new Unresolved(s"Vector index out of bounds:$i")
      else if (i < 0)
    ...
    case _ =>
      throw new Unresolved(s"Illegal vector index:$index")
  }
  ...
  def containsStrong(rhs: Val) = values.contains(rhs)
  ...
  def diff(rhs: Val) = rhs match {
    ...
    case x => Vector(values.filterNot(x.isEqualTo))
  }
  ...
  override def convertTo(destType: Class[_]) = {
    ...
    if (dt.isAssignableFrom(classOf[AnyRef])) toJava
      ...
      case _: Exception =>
        throw new EvalError(s"Cannot convert Vector into array of type '${dt.getName}´.")
    }
  }
