...
class NativeCallTest extends TestCase {
	...
	def testCallWithObj() {
		...
		scope.set("f", new JavaWrapper {
				def invoke(obj: java.lang.Double) = { obj * 5.5 }
			})
	...
	def testAmbigiousCall() {
		...
		scope.set("f", new JavaWrapper {
				def invoke(s: String) = { s + "!"}
				def invoke(b: Byte) = { b + 3}
			})
		scope.set("g", new JavaWrapper {
				def invoke(b: Short) = { b + 2 }
			})
	...
	def testMoreParams() {
		...
		scope.set("f", new JavaWrapper {
				def invoke() = { 42 }
				def invoke(s: String) = { s + "!"}
				def invoke(l: Long) = { l + 5}
				def invoke(b: Byte, d: Double) = { d + b}
			})
