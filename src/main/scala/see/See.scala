...
abstract class See extends Binding {
  ...
  /** Convenience method for {{{ set(name, new Long(v)) }}}
    */
  final def set(name: String, v: Int) {
    iset(name, Lint(v))
  }

  /** Convenience method for {{{ set(name, new Double(v)) }}} */
  final def set(name: String, v: Double) {
    iset(name, Real(v))
  }

  /** Convenience method for {{{ set(name, new Boolean(v)) }}} */
  final def set(name: String, v: Boolean) {
    iset(name, Bool(v))
  }
...
object See {
  ...
  private[see] var reflectionEnabled = true
  ...
  /** Enables Java reflection operator.
    ...
    * <br/>Therefore, you should disable it, if you cannot ensure that
...
/** A marker trait that is used to identify wrapped java methods.
  * A class that implements this trait is expected to provide one or more
  * methods called 'invoke' with zero to n arguments. An object of this type
  * may then be inserted into a [[see.Binding]]. When the object is
  * afterwards referenced from an expression, the evaluator
  * selects a matching method based on the number of arguments and converts the
  * argument type accordingly. Then, the method is invoked and its result
  * is converted back into a See type, which can be used for further evaluation.
  */
trait JavaWrapper {
  def name = "injected"
}