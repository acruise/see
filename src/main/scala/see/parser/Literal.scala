...
private object Literals {
  ...
  val BINARY = "0b[01][01_]*".r
  // matches binary literals
  val HEX = "(0x[0-9a-fA-F][0-9a-fA-F_]*L?)".r
  val RBINARY = "[01][01_]*".r
  // matches binary literals
...
private[see] trait Literals extends RegexParsers {
      ...
      if (s.contains('.') || s.contains('E') || s.contains('e')) {
        ...
        if (s.endsWith("L")) BigR(BigDecimal(s.substring(0, s.length - 1)))
          ...
          case _: Exception =>
            try {
              ...
              case _: Exception =>
                throw new ParseException("Invalid Real literal: " + s)
            }
        }
      } else {
        // Lint
        if (s.endsWith("L")) BigI(BigInt(s.substring(0, s.length - 1)))
          ...
          case _: Exception => try {
            ...
            case _: Exception =>
              throw new ParseException("Invalid Int literal: " + s)
          }
        }
      })
  }
    ...
    s => Constant(
      if (s.endsWith("L")) BigR(BigDecimal(s.substring(0, s.length - 1)))
        ...
        case _: Exception => try {
          ...
          case _: Exception => throw new ParseException("Invalid Real literal: " + s)
        }
      }
    )
  }
    ...
    s => Constant(
      if (s.endsWith("L")) BigI(BigInt(s.substring(0, s.length - 1)))
        ...
        case _: Exception => try {
          ...
          case _: Exception => throw new ParseException("Invalid Int literal: " + s)
        }
      })
  }
  ...
  //		}
  //	}
  final def cHex = immediate(RHEX) ^^ { s =>
    try {
      Constant {
        val hex = s.replaceAll("_", "")
        ...
        if (rhex.length > 16 || s.endsWith("L"))
          BigI(BigInt(rhex, 16))
        else if (rhex.length == 16) {
          // work around parsing problem with all-fs
          val tmp = JLong.parseLong(rhex.substring(0, 15), 16)
          val last = JLong.parseLong(rhex.substring(15), 16)
          Lint((tmp << 4) | last)
        }
      ...
      case _: Exception => throw new ParseException("Invalid hex literal: " + s)
    }
  }
