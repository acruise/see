...
private[see] abstract class Container
  ...
  def contains(rhs: Val): Boolean

  // works on keys in general!
  def containsStrong(rhs: Val): Boolean

  // works on keys in general!
  def containsWeak(rhs: Val): Boolean
  ...
  override def call(s: Scope, args: Val): Val = args match {
    case _ => map {
