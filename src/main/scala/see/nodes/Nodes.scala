...
private[see] case class Constant(v: Val) extends Final {
...
private[see] case class Variable(name: String) extends Final with LvNode {
  ...
  override def isDefinedIn(s: Scope) = (s getVar name).nonEmpty
...
private[see] case class Vnode(nodes: Seq[Node]) extends Final {
...
private[see] case class Nodes(nodes: Seq[Node]) extends Final {
  ...
  override def simplifyIn(s: Scope) = {
    if (nodes.isEmpty) EmptyNode
      ...
      for (node <- nodes) yield node.simplifyIn(s)
    }
  }
