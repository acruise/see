...
extends Atom(ref) {
	...
	override def evalIn(s: Scope) = {
		if (!see.See.reflectionEnabled)
			throw new EvalError("Java Reflection is diabled.")
	...
	private def derefClass(clsName: String, cargs: Array[Val]) = {
		val cls = try{Class.forName(clsName)} catch {
			case _ => throw new EvalError(
					"Unknown classname: '" + clsName + "'.")
		}
	...
	private def callMethod(cls: Class[_], 
		...
		val mths = for {m <- cls.getMethods 
						pt = m.getParameterTypes
						if (m.getName == methName) && 
						(pt.length == cargs.length)
		} yield m
		if (mths.isEmpty) throw new EvalError (
			"Class '" + cls.getName + 
			"' has no method " + methName + 
			" with %d arguments".format(cargs.length) 
		)
		...
		try { 
			...
			case x  => throw new EvalError(
					"Invocation of '" + cls.getName + 
					"." + methName + "' failed: " + x.getMessage)
		}
	}
	
	private def create(cls: Class[_], cargs: Array[Val]): Val = {
		val ctors = for (c <- cls.getConstructors; 
						 pt = c.getParameterTypes;
						 if pt.length == cargs.length)
							 yield c
		if (ctors.isEmpty) throw new EvalError (
			"Class '" + cls.getName + 
			"' has no constructor with %d arguments".format(cargs.length) 
		)
...
extends Atom(ref) with LvNode {
	...
	override def toString() = "(" + opd + ":>" + fieldName + ")"
	...
	private def jfield(cls: Class[_]) = try { 
		cls.getDeclaredField(fieldName) } catch {
		case _ => throw new EvalError (
				"Class '" + cls.getName + 
	...
	private def getField(cls: Class[_], inst: AnyRef): Val = {
		...
		try { 
			...
			case x  => throw new EvalError(
					"Cannot access '" + cls.getName + 
					"." + fieldName + "': " + x.getMessage)
		}
	}
