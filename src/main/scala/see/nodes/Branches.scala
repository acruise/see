  ...
  override def simplifyIn(s: Scope): Node = {
    ...
    if (ls.isInstanceOf[Constant] && rs.isInstanceOf[Constant]) {
      try {
        ...
        case x: Exception => if (s.isStableCheck) throw x
  ...
  override def finish(n: Node) = {
    n match {
      case relation: Relation =>
        val rel = new Relation(op, relation.rhs, rhs)
        Some(new Operation(BoolAnd, n, rel, PREC.BoolAnd))
      case _ =>
        Some(new Relation(op, n, rhs))
    }
  }
}
...
private[see] abstract class Assignment(val lv: LvNode, r: Node)
