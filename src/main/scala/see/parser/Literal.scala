		...
		s => Constant(
			if (s.contains('.') || s.contains('E') || s.contains('e')){
				...
				if (s.endsWith("L")) BigR(BigDecimal(s.substring(0, s.length-1)))
					...
					// retry as BigReal
					case _ => try{ BigR(BigDecimal(s)) } catch {
							case _ => throw new ParseException("Invalid Real literal: " + s)
						}
				}
			} else {
				// Lint
				if (s.endsWith("L")) BigI(BigInt(s.substring(0, s.length-1)))
					...
					// retry as BigInt
					case _ => try{ BigI(BigInt(s)) } catch {
							case _ => throw new ParseException("Invalid Int literal: " + s)
						}
				}
			})
	}
