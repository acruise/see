...
private class Scope(val parser: Parser, var parent: Parent = NoParent)
  ...
  final override def getNames: java.util.Set[String] = locals.keySet
  ...
  override def setParent(r: Resolver) {
    parent = Parent(r)
  }
  ...
  final def setResult(v: Val) = {
    locals.update(RNAME, v); v
  }
    ...
    if (CONSTNAME.matches(name)) {
      ...
      v.coerce match {
        case c: Callable if !c.isStable =>
          throw new EvalError(s"Cannot assign instable function to constant $name")
        case sv =>
          sv
      }
    } else v
