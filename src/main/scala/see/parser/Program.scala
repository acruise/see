...
private object Program {
	...
	private val TOKENS = Map[String, Generator](
		...
		"=>" -> new G(PREC.Anondef)(_.block ^^ { new FnodeP(_) }),
		":=" -> new G(PREC.Fundef)(_.block ^^ { new FundefP(_) }),
		"?=" -> stmtOpd(Return),
		"?!" -> stmtOpd(Assertion),
...
private[see] class Program extends Expressions {
	...
	final def patternmatch: Parser[Proto] =
		rep1sep(alternative, ":") ^^ { new MatcherP(_) }
	...
	final def alternative: Parser[Alternative] =
  	...
  	final def derefarg: Parser[Proto] = 
		("[^(!]+".r ~ (vector | "!")) ^^ {
			case n ~ "!" => new ReflectFieldP(n.trim) 
			case n ~ (a: Vnode) => new ReflectP(n.trim, a) 
		}
